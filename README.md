# Java Full Stack Learning & Projects 🚀

A monorepo containing all my full-stack Java + Spring Boot projects.  
Built with clean architecture, best practices, and real-world patterns that I’m learning and implementing step-by-step.

## 📂 Current Projects

| Project Folder                  | Description                                                                 | Status          | Tech Stack                              |
|---------------------------------|-----------------------------------------------------------------------------|------------------|------------------------------------------|
| `bank-management-system`        | Complete Banking Backend (REST API) – Users, Accounts, Transactions, Admin panel, Security | In Progress     | Spring Boot 3, JPA/Hibernate, MySQL, Lombok, MapStruct, Spring Security + JWT, Validation |
| `ecommerce-app`                 | (Coming Soon) Online Shopping Platform – Products, Cart, Orders, Payments   | Planned         | Spring Boot + React / Next.js            |
| `task-manager`                  | (Coming Soon) Advanced To-Do / Task Management App with Authentication      | Planned         | Spring Boot + React                      |

More projects will be added as I build them.

## 🏗️ Tech Stack (Common across projects)

- **Backend**: Java 21, Spring Boot 3.2+, Spring Web, Spring Data JPA, Hibernate
- **Database**: MySQL / PostgreSQL
- **Security**: Spring Security + JWT + Role-based Authorization
- **Mapping**: MapStruct (clean DTO ↔ Entity conversion)
- **Validation**: Jakarta Bean Validation
- **Tools**: Lombok, Spring Boot DevTools, Maven
- **Frontend** (future): React / Next.js + Tailwind / Shadcn UI
- **Deployment** (planned): Render, Railway, Vercel

## 📌 Project Goals & Learning Focus

- Master **clean layered architecture** (Controller → Service → Repository)
- Proper **DTO pattern** (separate request & response DTOs)
- Real-world **security implementation** (JWT, refresh tokens, roles)
- Transaction management & ACID properties
- Global exception handling & meaningful API responses
- Documentation & testing mindset
- Learn in public → share mistakes, learnings & code regularly
